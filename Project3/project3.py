...
def nn_layer(input_tensor, input_dim, output_dim, layer_name, act=tf.nn.relu6):
...
hidden1 = nn_layer(x, 100, 400, 'layer1')
hidden1_dropped = tf.nn.dropout(hidden1, keep_prob)
#hidden2 = nn_layer(hidden1_dropped,1000,500,'layer2', act=tf.nn.relu)
#hidden2_dropped = tf.nn.dropout(hidden2, keep_prob)
y = nn_layer(hidden1_dropped, 400, 5, 'layer3', act=tf.nn.softmax)
...
print(y_edit)
Xtrain, Xtest, Ytrain, Ytest = skcv.train_test_split(X, y_edit, train_size=0.8)
