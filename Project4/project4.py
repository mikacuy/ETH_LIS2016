...
hidden1 = nn_layer(x, 128, 700, 'layer1')
hidden1_dropped = tf.nn.dropout(hidden1, keep_prob)
hidden2 = nn_layer(hidden1_dropped,700,300,'layer2', act=tf.nn.relu)
hidden2_dropped = tf.nn.dropout(hidden2, keep_prob)
y = nn_layer(hidden2_dropped, 300, 10, 'layer3', act=tf.nn.softmax)
...
y_edit=np.zeros((len(Y),10),dtype=np.int)
...
with tf.Session() as sess:
    ...
    write_output_file(y_predict,"project4_relu.csv")
